...
2. Completa los espacios marcados con ___ usando los conceptos de manejo de archivos y procesamiento de CSV apropiados.
3. No modifiques los nombres de las funciones existentes.
4. Puedes usar las bibliotecas csv y json que ya están importadas.
...
def crear_csv_ejemplo(nombre_archivo):
    ...
    with open(nombre_archivo, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(['Fecha', 'Vendedor', 'Producto', 'Cantidad'])
        writer.writerows([
...
def leer_csv(nombre_archivo):
    """
    Lee un archivo CSV y retorna sus contenidos como una lista de diccionarios.
    
    Parámetros:
    - nombre_archivo (str): El nombre del archivo CSV a leer.
    
    Retorna:
    - list: Una lista de diccionarios, donde cada diccionario representa una fila del CSV.
    """
...
def calcular_ventas_totales(datos):
    """
    Calcula las ventas totales a partir de los datos del CSV.
    
    Parámetros:
    - datos (list): Lista de diccionarios con los datos de ventas.
...
def encontrar_mejor_vendedor(datos):
    """
    Encuentra al vendedor con más ventas.
    
    Parámetros:
    - datos (list): Lista de diccionarios con los datos de ventas.
    
    Retorna:
    - str: El nombre del mejor vendedor.
    """
    ventas_por_vendedor = {}
    for fila in datos:
        vendedor = fila['Vendedor']
        cantidad = int(fila['Cantidad'])
        ventas_por_vendedor[vendedor] = ventas_por_vendedor.get(vendedor, 0) + cantidad #ventas_por_vendedor[vendedor] + cantidad
...
def ventas_por_producto(datos):
    ...
    Retorna:
    - dict: Un diccionario con el total de ventas por producto.
    """
    ventas = {}
    for fila in datos:
...
def guardar_resumen_json(nombre_archivo, resumen):
    """
    Guarda el resumen de ventas en un archivo JSON.
    
    Parámetros:
    - nombre_archivo (str): El nombre del archivo JSON a crear.
    - resumen (dict): Un diccionario con el resumen de ventas.
    """
...
def procesar_ventas(archivo_entrada, archivo_salida):
    """
    Procesa el archivo de ventas y genera un resumen en JSON.
    
    Parámetros:
    - archivo_entrada (str): El nombre del archivo CSV de entrada.
    - archivo_salida (str): El nombre del archivo JSON de salida.
    """
